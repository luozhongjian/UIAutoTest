...
def do_action(): 
    ...
    #制文
    documentrep = approval(driver, testDataDic)
    documentRes = documentrep.make_document()
    if documentRes:
        reporter.reportEvent("Passed", u"建设项目制文", u"制文流程成功！")
    else:
        reporter.reportEvent("Failed", u"建设项目制文", u"制文流程失败！")
...
def sys_check(): 
    ...
    driver.switch_to_frame(driver.find_element_by_xpath("//*[@id='page_tab1_index7']"))  
    
    #判断控件是否可见
    NTKO = driver.find_elements_by_xpath("//*[@id='TANGER_OCX']")
    if len(NTKO) == 0:
        reporter.reportEvent("Failed", u"查找批文框架", u"不能正常加载批文" )  
    if len(NTKO) == 1:
        reporter.reportEvent("Passed", u"查找批文框架", u"能正常加载批文")  
