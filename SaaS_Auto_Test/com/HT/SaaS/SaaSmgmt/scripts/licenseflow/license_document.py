...
def do_action(): 
    ...
    #许可证制文
    documentrep = License(driver, testDataDic)
    documentRes = documentrep.license_make_document()
    if documentRes:
        reporter.reportEvent("Passed", u"许可证制文", u"制文流程成功！")
    else:
        reporter.reportEvent("Failed", u"许可证制文", u"制文流程失败！")
...
def sys_check(): 
    ...
    sleep(1)
    driver.find_element_by_xpath("//li[text()='%s']" % (newstype[4])).click()
    sleep(1)
    ...
    try:
        if driver.find_element_by_xpath("//*[@id='dataBasic|2|r1001|c113']/div").text == newstype[4]:
        
            reporter.reportEvent("Passed", u"报件状态一致", newstype[4] )   
        else:
            reporter.reportEvent("Failed", u"报件状态不一致", newstype[4]) 
    except:
    ...
    #查看许可证打印界面
    driver.find_element_by_xpath("//td[text()='许可证打印']").click()
    sleep(1)
    driver.switch_to_frame(driver.find_element_by_xpath("//*[@id='page_tab1_index6']"))  
    
    #将许可证打印界面的数据存入字典
    licenseNum = driver.find_elements_by_xpath("//*[@id='dataBasicgrid']/div[4]/div[2]/div/table//tr//td[1]/div")
    licensetype = driver.find_elements_by_xpath("//*[@id='dataBasicgrid']/div[4]/div[2]/div/table//tr//td[2]/div")
    
    LicenseInfoDic = {}
    i = 0
    for i in range(len(licenseNum)):
            LicenseInfoDic[licenseNum[i].text.strip()] = licensetype[i].text
    
    #循环字典的key,value，写入Excel
    for key, value in LicenseInfoDic.items():
    ...
    #流程节点的状态
    amscheck = driver.find_element_by_xpath("//div[@nodekey='approve_004']")

    if amscheck.get_attribute('checkstart')=="do":
        reporter.reportEvent("Passed", u'判断流程节点', u'制文节点状态为' + amscheck.get_attribute('checkstart'))
    else:
        reporter.reportEvent("Failed", u'判断流程节点', u'制文节点状态为' + amscheck.get_attribute('checkstart'))
