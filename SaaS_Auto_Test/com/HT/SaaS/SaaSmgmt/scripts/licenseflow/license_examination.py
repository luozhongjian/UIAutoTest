...
def do_action(): 
    ...
    #审批
    examinationrep = License(driver, testDataDic)
    examinationRes = examinationrep.license_examination()
    if examinationRes:
        reporter.reportEvent("Passed", u"许可证审批", u"审批流程成功！")
    else:
        reporter.reportEvent("Failed", u"许可证审批", u"审批流程失败！")
...
def sys_check(): 
    ...
    sleep(1)
    driver.find_element_by_xpath("//li[text()='%s']" % (newstype[3])).click()
    sleep(1)
    ...
    try:
        if driver.find_element_by_xpath("//*[@id='dataBasic|2|r1001|c113']/div").text == newstype[3]:
        
            reporter.reportEvent("Passed", u"报件状态一致", newstype[3] )   
        else:
            reporter.reportEvent("Failed", u"报件状态不一致", newstype[3] ) 
    except:
    ...
    if amscheck.get_attribute('checkstart')=="yes":
        reporter.reportEvent("Passed", u'判断下一步流程节点', u'制文节点状态为' + amscheck.get_attribute('checkstart'))
    else:
        reporter.reportEvent("Failed", u'判断下一步流程节点', u'制文节点状态为' + amscheck.get_attribute('checkstart'))
